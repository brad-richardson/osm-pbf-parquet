...
joined = xml_df.set_index(["id", "type"]).join(pq_df.set_index(["id", "type"]), on=["id", "type"], how="outer", rsuffix="_pq", validate="one_to_one")
...
if pq_missing.count().sum() > 0:
    ...
    raise AssertionError(f"Unexpected missing rows from parquet, count: {pq_missing.count().sum()}")
...
if xml_missing.count().sum() > 0:
    ...
    raise AssertionError(f"Unexpected extra rows in parquet, count: {xml_missing.count().sum()}")

print("Checking mismatched scalar values")
mismatched = joined[~((joined['version'].eq(joined['version_pq']))
...
if mismatched.count().sum() > 0:
    ...
    mismatched['timestamp_match'] = mismatched['timestamp'].eq(mismatched['timestamp_pq'])
    ...
    print(mismatched[[
        'version', 'version_pq', 'version_match',
        'timestamp', 'timestamp_pq', 'timestamp_match',
        'lat', 'lat_pq', 'lat_match',
        'lon', 'lon_pq', 'lon_match'
        ]].head())
...
mismatched_tags = joined[~(joined['tag'].apply(lambda tags: remap_tags(tags)).eq(joined['tags']))]
...
def compare_nodes(xml_nodes, pq_nodes):
    for (xml, pq) in zip(xml_nodes, pq_nodes):
        if str(xml) != str(pq):
            return False
    return True
joined['nd_compare'] = joined.apply(lambda row: compare_nodes(row['nd_remap'], row['nds_remap']), axis=1)
...
def remap_members(members):
    ...
    if type(members) == dict:
        return [(members.get('@type', members.get('type')), int(members.get('@ref', members.get('ref'))), members.get('@role', members.get('role')))]
    return [(member.get('@type', member.get('type')), int(member.get('@ref', member.get('ref'))), member.get('@role', member.get('role'))) for member in members]
...
def compare_members(xml_members, pq_members):
    for (xml, pq) in zip(xml_members, pq_members):
        if xml != pq:
            return False
    return True
joined['members_compare'] = joined.apply(lambda row: compare_members(row['member_remap'], row['members_remap']), axis=1)
...
if mismatched_members.count().sum() > 0:
    ...
    raise AssertionError(f"Mismatched members, count: {mismatched_members.count().sum()}")
